...
+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar>*)registrar {
    FlutterMethodChannel* channel = [FlutterMethodChannel
      methodChannelWithName:@"plugins.hunghd.vn/image_cropper_plus"
...
- (void)setupUiCustomizedOptions:(id)options forViewController:(TOCropViewController*)controller {
    ...
    NSNumber *doneButtonTitleColor = options[@"ios.done_button_title_color"];
    NSNumber *cancelButtonTitleColor = options[@"ios.cancel_button_title_color"];
    ...
    if (doneButtonTitleColor && [doneButtonTitleColor isKindOfClass:[NSNumber class]]) {
        /// From (https://stackoverflow.com/questions/10062660/converting-nsnumber-to-nsinteger)
        NSInteger rgbValue = [doneButtonTitleColor integerValue];
        controller.doneButtonColor = UIColorFromRGB(rgbValue);
    }
    
    
    if (cancelButtonTitleColor && [cancelButtonTitleColor isKindOfClass:[NSNumber class]]) {
        /// From (https://stackoverflow.com/questions/10062660/converting-nsnumber-to-nsinteger)
        NSInteger rgbValue = [cancelButtonTitleColor integerValue];
        controller.cancelButtonColor = UIColorFromRGB(rgbValue);
    }
...
- (void)cropViewController:(TOCropViewController *)cropViewController didCropToImage:(UIImage *)image withRect:(CGRect)cropRect angle:(NSInteger)angle
{
    ...
    if ([@"png" isEqualToString:_compressFormat]) {
        ...
        tmpFile = [NSString stringWithFormat:@"image_cropper_plus_%@.png", guid];
    } else {
        data = UIImageJPEGRepresentation(image, _compressQuality);
        tmpFile = [NSString stringWithFormat:@"image_cropper_plus_%@.jpg", guid];
    }
